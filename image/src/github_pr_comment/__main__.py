...
def current_user(actions_env: GithubEnv) -> str:
    token_hash = hashlib.sha256(f'POps-Rox/tf-gh-actions/{github_token}'.encode()).hexdigest()
...
''')
    ...
    if sys.argv[1] == 'plan':
        ...
        if comment.comment_url is None:
            ...
            sys.stdout.write("Generate the plan first using the POps-Rox/tf-gh-actions/terraform-plan action. Alternatively set the auto_approve input to 'true'\n")
            sys.stdout.write("If POps-Rox/tf-gh-actions/terraform-plan was used with add_github_comment set to changes-only, this may mean the plan has since changed to include changes\n")
...
""")
            ...
            if plan_ref := comment.headers.get('plan_job_ref'):
                sys.stdout.write(f'\nCompare with the plan generated by the POps-Rox/tf-gh-actions/terraform-plan action in {plan_ref}\n')
