...
- **Cloud Provider:** Cloud-agnostic (actions support any Terraform backend; base image includes Azure CLI)
- **IaC Tool:** Terraform (this project builds GitHub Actions for Terraform)
- **Container Runtime:** Docker (shared Dockerfile at `image/Dockerfile`)
- **Orchestration:** N/A (single Docker image, no orchestrator)
- **Build Command:** `docker build -t tf-actions image/`
- **Deploy Command:** `make release VERSION=vX.Y.Z` (tags + GitHub Release)
